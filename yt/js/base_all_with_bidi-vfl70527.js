...
var cookie_prefix='';
var gPixelGif="https://web.archive.org/web/20090105205935/http://s.ytimg.com/yt/img/pixel-vfl73.gif";
function _gel(id){
...
this.getFlag2=function(flag){return _getFlag('f2',flag);};
this.setFlag2=function(flag,bit){return _setFlag('f2',flag,bit);};
...
var UT_RATING_IMG_HOVER='https://web.archive.org/web/20090105205935/http://s.ytimg.com/yt/img/star_hover-vfl2056.gif';
var UT_RATING_IMG_HALF='icn_star_half_19x20';
var UT_RATING_IMG_BG='icn_star_empty_19x20';
var UT_RATING_IMG_REMOVED='https://web.archive.org/web/20090105205935/http://s.ytimg.com/yt/img/star_removed-vfl2028.gif';
...
previewVisible="style='display: inline'";
}else{
...
var innerHTMLContent='\
	<form name="'+reply_comment_form+'" id="'+reply_comment_form+'" onSubmit="return false" method="post" action="/comment_servlet?add_comment=1&comment_type='+comment_type+'" >\
		...
		></textarea>\
		<input style="vertical-align:top; margin-left: 10px" align="left" type="button" name="preview_comment_button" \
				value="Audio Preview"'+previewVisible+' \
...
var temp_HTML='<img src="https://web.archive.org/web/20090105205935/http://s.ytimg.com/yt/img/icn_loading_animated-vfl24663.gif">';
...
setInterval(pollLocationHash,1000);
...
function post_bulletin(text,video_id){
var onSuccess=function(){
showDiv('post_bulletin_success');
hideDiv('post_bulletin_box');
};
var onFailure=function(){
hideDiv('post_bulletin_box');
showDiv('post_bulletin_failure');
};
showDiv('post_bulletin_loading');
postAjaxForm('post_bulletin_box','postBulletinForm',
...
function customizeEmbed(isWidescreenVideo){
...
var oldTheme=_gel('watch-embed-size-radio-'+selectedSize+(isWidescreen?'-wide':''));
var newTheme=_gel('watch-embed-size-radio-'+newSize+(isWidescreen?'-wide':''));
yt.UserPrefs.set('ems',newSize);
yt.UserPrefs.save();
removeClass(oldTheme,'radio_selected');
addClass(newTheme,'radio_selected');
newTheme.blur();
selectedSize=newSize;
var hackText=_gel("watch-customize-embed-size");
hackText.innerHTML+=" ";
return false;
}
...
function onUpdateEmbedSizeDisplay(){
var customEmbedSizes=(isWidescreen)?gCustomEmbedSizesWide:gCustomEmbedSizes;
for(var x in customEmbedSizes){
var sizeText=_gel("watch-embed-size-text-"+x);
var embedSizes=getEmbedSize(x);
sizeText.innerHTML=embedSizes[0]+"x"+embedSizes[1];
}
}
...
yt.UserPrefs.setFlag(yt.UserPrefs.Flags.FLAG_EMBED_NO_RELATED_VIDEOS,!related);
yt.UserPrefs.save();
}
...
if(ref('watch-channel-videos-panel')){yt.UserPrefs.setFlag(yt.UserPrefs.Flags.FLAG_WATCH_EXPAND_MOREFROM_PANEL,isPanelExpanded(ref('watch-channel-videos-panel')));}
...
var moreDiv=_gel('watch-related-video-list-more-div');
if((loadingDiv==null)||(loadingDiv.style.display!="none")){
return;
}
loadingDiv.style.display="block";
moreDiv.style.display="none";
postUrlXMLResponse("/related_ajax?action_get_more_related=1",
"v="+pageVideoId+"&search_query="+encodeURIComponent(pageVideoSearchQueryStr)+"&start="+pageFillRelatedVideoStartIndex,
function(result){
var loadingDiv=_gel('watch-related-video-list-loading-div');
var moreDiv=_gel('watch-related-video-list-more-div');
...
_gel(resultsDivName).innerHTML="<img src='https://web.archive.org/web/20090105205935/http://s.ytimg.com/yt/img/LoadingGraphic-vfl3869.gif'>";
if(_gel("watchSearchSubmit")){
...
var previewSwf=new SWFObject('https://web.archive.org/web/20090105205935/http://s.ytimg.com/yt/swf/textreader-vfl58814.swf',"preview_comment","1","1",8);
...
var quoted_negative_queries_array=[];
var first_index;
while((first_index=negative_queries.indexOf('"'))!=-1){
var second_index=negative_queries.indexOf('"',first_index+1);
if(second_index==-1){
break;
}else{
var the_negative_query=negative_queries.substring(first_index,second_index+1);
if(the_negative_query.length>2)
quoted_negative_queries_array.push('-'+the_negative_query);
negative_queries=negative_queries.replace(the_negative_query,"");
}
}
if(quoted_negative_queries_array.length>0){
search_query+=' '+quoted_negative_queries_array.join(' ');
}
...
if(_gel('video_grid')){
removeClass(_gel('video_grid'),viewType=='L'?'grid-view':'list-view');
addClass(_gel('video_grid'),viewType=='L'?'list-view':'grid-view');
}
...
var videoEpisode=getElementsByTagNameAndClass('div','video-episode',toEl)[0];
if(showAll){
addClass(videoEpisode,"showepisode");
}else{
removeClass(videoEpisode,"showepisode");
}
}
function getLoadingEpisodeDiv(fromEl,id){
var loadingEl=document.createElement('div');
loadingEl.id=id;
loadingEl.className="video-entry loading-episode";
...
if(getElementsByTagNameAndClass('div','video-episode-return',element).length==0){
removeClass(element,"original-episode");
var ret=document.createElement("div");
ret.className="video-episode-return yellowBox";
ret.innerHTML=gReturnToOriginalEpisodeLink;
element.appendChild(ret);
}
}
function onOriginalEpisode(element){
var videoEntry=findAncestorByClass(element,"video-entry");
var videoCell=findAncestorByClass(videoEntry,"video-cell");
var originalVideo;
var elements=getElementsByTagNameAndClass('div','original-episode',videoCell);
if(elements.length==1){
originalVideo=elements[0].id;
elements=getElementsByTagNameAndClass('a','episode-link',videoEntry);
...
var fromEl=findAncestorByClass(clicked_episode,"video-entry");
var fromVideo=fromEl.id.replace(/^v_/,"");
var toVideo=clicked_episode.id.replace(/^episode_/,'');
var toEl=_gel('v_'+toVideo);
var show_all=hasClass(getElementsByTagNameAndClass('div','video-episode',fromEl)[0],"showepisode");
var videoCell=fromEl.parentNode;
if(toEl){
if(videoCell==toEl.parentNode){
toEl=videoCell.removeChild(toEl);
}else{
...
fromEl=videoCell.replaceChild(toEl,fromEl);
fromEl.style.display="none";
videoCell.appendChild(fromEl);
if(fromEl.id.match("clone$")){
videoCell.removeChild(fromEl);
}
gLoadingEpisode=false;
thumbnailDelayLoad.loadImages();
}else{
var ld=getLoadingEpisodeDiv(fromEl,fromVideo+"_loading");
videoCell.appendChild(ld);
fromEl=videoCell.replaceChild(ld,fromEl);
fromEl.style.display="none";
videoCell.appendChild(fromEl);
...
var elements=getElementsByTagNameAndClass('div','video-entry',tmpParent);
for(var i in elements){
var v=elements[i];
if(v.id=="v_"+toVideo){
appendReturnToOriginalLink(v);
setEpisodeStatus(v,show_all);
videoCell.replaceChild(v,ld);
if(fromEl.id.match("clone$")){
fromEl.parentNode.removeChild(fromEl);
}
}else{
v.style.display="none"
videoCell.appendChild(v);
}
}
if(!_gel("v_"+toVideo)){
...
postUrlXMLResponse('/results_ajax?action_change_episode=1',urlEncodeDict(data),onSuccess);
}
}
...
script.setAttribute("src","https://web.archive.org/web/20090105205935/http://maps.google.com/maps?file=api&v=2.x&key="+gMapsApiKey+"&c&async=2&callback="+callbackName+"&hl="+mapLanguage);
...
postUrlXMLResponse('/results_ajax?geo_ob=1&action_geosearch_onebox=1',urlEncodeDict(data),onSuccess);
}
...
postUrlXMLResponse("/watch_queue_ajax?action_clear_queue=1",append_token_for_queue(),watchQueueCleared);
var suffix="QL";
...
postUrlXMLResponseJSON("/watch_queue_ajax?action_add_to_queue=1&video_id="+nextVideoId,
...
isRemoving=true;
postUrlXMLResponse("/watch_queue_ajax?action_remove_from_queue=1&video_id="+removelist[removelist.length-1],append_token_for_queue(""),self.watchVideoRemoved);
removelist.pop();
}
isRemoving=false;
...
function getNextListRow(loopPlayback,offset,forceNext){
if(!playnextFrom||!(watchIsPlayingAll||forceNext)){
return null;
}
var offset=typeof(offset)!='undefined'?offset:1;
...
if(hasClass(currentRow,'watch-playlist-row-playing')){
var index=(i+offset)%videosInPlaylist;
if(index<0){
index=videosInPlaylist+index;
}
if(loopPlayback||index==(i+offset)){
nextRow=_gel(getId('playlistRow',playnextFrom,index));
}else{
nextRow=null;
}
break;
...
function gotoNext(loopPlayback,suffix,forceNext){
var forceNext=typeof(forceNext)=='undefined'?false:forceNext;
var nextRow=getNextListRow(loopPlayback,1,forceNext);
if(nextRow){
if(suffix==null){
suffix="";
}
...
function getNextVideoId(offset){
if(offset>21){
return '';
}
var nextRow=getNextListRow(false,offset);
if(nextRow){
var nextVideoId=getNodeVideoId(nextRow);
if(nextVideoId){
return nextVideoId;
}
}
...
var autoNextComponents=['PL','QL','TL','RL'];
...
function findPlaylistRowByVideoId(encryptedId,suffix){
var videosInPlaylist=getPlaylistVideoCount(suffix);
for(var i=0;i<videosInPlaylist;i++){
var videoRow=_gel(getId("playlistRow",suffix,i));
...
return findPlaylistRowByVideoId(encryptedId,'QL');
}
...
fillQuicklistRow(newRow,videoCount,"","https://web.archive.org/web/20090105205935/http://s.ytimg.com/yt/img/pixel-vfl73.gif",MSG_Loading,"","");
...
postUrlXMLResponseJSON("/watch_queue_ajax?action_get_all_queue_videos_component=1&v="+pageVideoId+"&al=1",append_token_for_queue("watch3=1"),handleWatchQueueGet);
}else{
...
if(removelist.length>0){
postUrlXMLResponse("/watch_queue_ajax?action_remove_from_queue&video_id="+removelist[removelist.length-1],append_token_for_queue(),self.videoRemoved);
removelist.pop();
}
}
...
postUrlXMLResponse("/watch_queue_ajax?action_clear_queue=1",append_token_for_queue(),function(){window.location="watch_queue?all";});
}
function set_pop_status(pop_status){
postUrlXMLResponse("/watch_queue_ajax?action_set_pop_status=1&pop_videos="+pop_status,append_token_for_queue(),function(){})
}
...
this.addImage(new ytImage("https://web.archive.org/web/20090105205935/http://s.ytimg.com/yt/img/pixel-vfl73.gif","","&nbsp;","","&nbsp;","",true));
}
}
...
if(window['jstiming']['fmt']){
extra+="&fmt="+window['jstiming']['fmt'];
}
if(opt_extraParams){
...
new Image().src=['https://web.archive.org/web/20090105205935/http://csi.gstatic.com/csi?v=2&s=youtube&action=',action,
...
window['jstiming']['loaded_continuous_thumbnail_count']=0;
window['jstiming']['thumbnail_loaded']=function(index){
var count=++window['jstiming'].loaded_thumbnail_count;
if((typeof(index)!='undefined')&&(index<4))
window['jstiming'].loaded_continuous_thumbnail_count++;
if(window['jstiming'].loaded_continuous_thumbnail_count==4)
window['jstiming'].tick('tn_c4');
if(count!=1&&count!=5&&count!=10&&count!=20&&count!=30)
return;
window['jstiming'].tick('tn'+count);
}
}
function EDGETEST_pingOneDomain(domain,path){
(new Image()).src='http://'+domain+path;
}
function EDGETEST_pingDomains(service,referenceDomain,checkedDomain){
var currentDate=new Date();
var randomNumber=Math.random();
var suffix='/gen_204?atyp='+service+'&id='+
currentDate.getTime().toString(32)+
randomNumber.toString(16);
if(randomNumber>=0.5){
EDGETEST_pingOneDomain(referenceDomain,suffix);
EDGETEST_pingOneDomain(checkedDomain,suffix);
}else{
EDGETEST_pingOneDomain(checkedDomain,suffix);
EDGETEST_pingOneDomain(referenceDomain,suffix);
}
}
function EDGETEST_pingAllAndCore(){
EDGETEST_pingDomains('edge','coretest.ytimg.com','alltest.ytimg.com');
}
...
/*
     FILE ARCHIVED ON 20:59:35 Jan 05, 2009 AND RETRIEVED FROM THE
     INTERNET ARCHIVE ON 20:49:55 Jan 19, 2025.
...
playback timings (ms):
  ...
  esindex: 0.009
  cdx.remote: 5.729
  LoadShardBlock: 96.375 (3)
  PetaboxLoader3.datanode: 117.27 (4)
  load_resource: 116.083
  PetaboxLoader3.resolve: 75.794
*/